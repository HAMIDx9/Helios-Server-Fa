...
class Election(HeliosModel):
  ...
  def pretty_eligibility(self):
    if not self.eligibility:
      return "هر فرد می‌توانید رای دهد"
    else:
      return_val = "<ul>"
  ...
  def issues_before_freeze(self):
    ...
    if self.questions == None or len(self.questions) == 0:
      issues.append(
        {'type': 'questions',
         'action': "اضافه کردن سوال به ورقه‌رای"}
        )
    ...
    if len(trustees) == 0:
      issues.append({
          'type': 'trustees',
          'action': "اضافه کردن حداقل یک متولی"
          })
    ...
    if self.voter_set.count() == 0 and not self.openreg:
      issues.append({
          "type" : "voters",
          "action" : 'لیست رای‌دهندگان را وارد کنید(یا انتخابات را عمومی کنید)'
          })
